...
class alipay
{
    ...
    function get_code($order, $payment)
    {
        ...
        switch ($real_method){//接口类型
            case '0':
                $service = 'trade_create_by_buyer';//双向接口
                break;
            case '1':
                $service = 'create_partner_trade_by_buyer';//担保交易接口
                break;
            case '2':
        ...
        $parameter = array(
            ...
            /* 业务参数 */
            'subject'           => $order['top_order_sn'],
            'out_trade_no'      => $order['top_order_sn'] . $order['log_id'],
    ...
    function respond()
    {
        ...
        /*这里存在严重问题，比如out_trade_no:10108  subject:10  结果就会变成 8*/
        $pay_log_id =str_replace_once($_GET['subject'], '', $_GET['out_trade_no']);
        //$pay_log_id = str_replace($_GET['subject'], '', $_GET['out_trade_no']);//subject是订单号，out_trade_no是订单号加pay_log表ID
        $pay_log_id = trim($pay_log_id);
        ...
        {
            /* 改变订单状态 */
            order_paid($order_sn,$pay_log_id, 2);
        ...
        {//交易成功
            /* 改变订单状态 */
            order_paid($order_sn,$pay_log_id);
