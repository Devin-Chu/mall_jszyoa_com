...
//    $smarty->assign('back_act', $back_act);
    //分店是否开启
    $smarty->assign('shops_mamageType', $_CFG['shops_mamageType']);
    //省
    ...
    /* 插件ch 分店管理 添加 S */
    $sql='SELECT id,shops FROM '.$ecs->table('shops_mamage');
    $shopsInfo=$db->getAll($sql);
    $smarty->assign("shopsInfo",$shopsInfo);
...
elseif ($action == 'act_register')
{
    ...
    else
    {
        ...
        $other['market_id']=isset($_POST['shopsMamageMarket']) ? intval($_POST['shopsMamageMarket']) : 0 ;//分店
        ...
        //$other['mobile_phone'] = isset($_POST['extend_field5']) ? $_POST['extend_field5'] :unset($other['mobile_phone']);
...
///rec_id其实是user_account表中的充值记录对应ID
        //取得支付信息，生成支付代码
        ...
        $order['top_order_sn']       = $surplus['rec_id'];//Luyin增加，因为pay_log表中的sn 号变量名称使用了top_order_sn
        ...
        //记录支付log
        $order['log_id'] = insert_pay_log($surplus['rec_id'],$surplus['rec_id'], $order['order_amount'], $type=PAY_SURPLUS, 0);
...
elseif ($action == 'pay')
{
    ...
    //变量初始化
    $surplus_id = isset($_GET['id'])  ? intval($_GET['id'])  : 0;
    $payment_id = isset($_GET['pid']) ? intval($_GET['pid']) : 0;//支付方式ID
...
elseif ($action == 'save_order_address')
{
    ...
    $vd->set_rules('consignee', '收货人姓名',  'require','收货人不能为空！');
    $vd->set_rules('address', '详细地址',  'require','详细地址不能为空！');
    $vd->set_rules('zipcode', '邮编',  'require','邮编不能为空！');
    $vd->set_rules('email', '邮箱验证', 'email', '输入的的邮箱地址不正确！');
    $vd->set_rules('mobile', '手机号码验证', "mobile|require", '您输入的手机号码为空或不存在！');
  //  $vd->set_rules('tel', '电话', "require", '电话话码不能为空！');
    if (!$vd->is_valid($_POST))
    {
        foreach($vd->get_errors() as $val){
            $errors.=$val."<br>";
        }

        show_message($errors, $links = '', $hrefs = '', $type = 'warning', $auto_redirect = true);
    }
